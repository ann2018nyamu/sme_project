 ...
 */
public class PurchaseOrderInventoryStockAdapter extends RecyclerView.Adapter<PurchaseOrderInventoryStockAdapter.ViewHolder> {

    List<InventoryStock> inventoryStockList;
    
    Context context;
    PoInventoryComm poInventoryComm;
     ...
     * @param fragment  the order list fragment instance
     */
    public PurchaseOrderInventoryStockAdapter(Context context, List<InventoryStock> inventoryStockList, OrderListFragment fragment) {
        this.inventoryStockList = inventoryStockList;
        this.context = context;
        this.poInventoryComm=  fragment;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_porder_item, parent, false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull PurchaseOrderInventoryStockAdapter.ViewHolder holder, int position) {
    ...
    /**
     * adds quantity to single item list
     * @param holder
     */
    private void addQty(ViewHolder holder){
        ...
        poInventoryComm.poInventoryStockMessage(inventoryStockList.get(holder.getAdapterPosition()),"update",holder.getAdapterPosition());
        notifyItemChanged(holder.getAdapterPosition());
    }
    /**
     * removes/subtract quantity to single item list
    ...
    private void removeQty(ViewHolder holder){
        ...
        initialCount =  initialCount -1;
        if(initialCount == 0){
            poInventoryComm.poInventoryStockMessage(inventoryStockList.get(holder.getAdapterPosition()),"remove",holder.getAdapterPosition());
            return;
        }
        holder.etxtInventoryStockQty.setText(initialCount.toString());
        inventoryStockList.get(holder.getAdapterPosition()).setChoosenquantity(initialCount);
     ...
     */
    public interface PoInventoryComm{
        /**
         * Str message.
         *
         ...
         */
        void poInventoryStockMessage(InventoryStock inventoryStock, String action,Integer position);
    }
